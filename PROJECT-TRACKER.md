...
| 2026-08-05 | E2E verification of automatic document generation: full workflow (service + template + mappings → kiosk request → status 5 auto-generate → download). Fixed `form_data` not parsed from MySQL JSON string in document.service.js — application-form placeholders rendered blank | backend/src/services/document.service.js, docs/tasks/00-core/PDMP-007-PROJECT-CHANGELOG.md |

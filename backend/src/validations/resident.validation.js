...
const createValidation = [
  ...
  body('birthPlace').optional().trim().isLength({ max: 150 }).withMessage('Birth place must not exceed 150 characters.'),
  ...
  body('email').optional({ values: 'falsy' }).trim().isEmail().withMessage('Invalid email format.').isLength({ max: 100 }).withMessage('Email must not exceed 100 characters.').normalizeEmail(),

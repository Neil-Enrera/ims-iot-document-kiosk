...
- **Kiosk screen redesign (phase 2 — document flow)** — replaced the dark blue kiosk theme with a light theme (`#F8FAFC` background, slate text, orange `#F97316` accent) and staggered layout pieces across the document request flow:
  ...
  - **Landing + Continue Without Barangay ID responsiveness** — both screens follow the same responsive pattern: the two main cards use the landing page's `flex-wrap` layout (`flex-1 min-w-[min(100%,320px)] max-w-[560px]`) so they stay on one row and scale/skew gracefully instead of snapping to a fixed grid, and both footers were compacted (tighter padding/spacing, smaller icons + labels) so they no longer crowd the main content.
  - **Unified Back control** — all light-theme kiosk screens (RFID scan, Continue Without Barangay ID, Enter Your Information) now share one compact Back pill: a circular arrow badge + "Back" label in a single clickable button, fixed/absolute top-left, consistent spacing and typography.

...
| 2026-08-13 | OpenCode | Reworked kiosk submission into a transaction-grouped flow (migration 020). Every submission now creates one `transactions` row (`TXN-YYYY-NNNNN`, optional `guest_snapshot`, unique `idempotency_key`) that anchors one or more `requests` rows. Applied migration 020 to the dev DB (creates `transactions`, `requests.transaction_id`, and `services` policy flags `can_combine_with_others` / `allow_multiple_active_requests` / `allow_new_request_after_release`). New `transaction.repository.js` + `transaction.service.js`; `POST /kiosk/requests` now routes through `submitTransaction` (idempotent whole-submission retry returns the existing transaction; per-service resident duplicate/repeat policy blocks with `ACTIVE_REQUEST_EXISTS` / `NO_REPEAT_AFTER_RELEASE`; guest identity matching is advisory via `possible_duplicates`). Response stays request-number compatible (`request_number` = first request) plus `transaction_number`/`requests`/`duplicate`. Verified: 78/78 backend tests, ESLint clean (pre-existing warnings only), frontend `tsc --noEmit` clean, and live smoke tests for guest/resident submit, idempotency, duplicate policy, and advisory guest matching (test rows removed after). |